...
if not redis:get(veer..'group:add'..msg.chat_id_) then return sendMsg(msg.chat_id_,msg.id_,'- المجموعه بالتاكيد ✓️ تم تعطيلها \n- بواسطه ⋙「 '..NameUser..' 」 \n') end  
...
KleshaID = KleshaID:gsub("{البوت}",redis:get(veer..':NameBot:'))
...
redis:del(veer..":RtbaNew8:"..msg.chat_id_)
...
local Rtba8 = redis:get(veer..":RtbaNew8:"..msg.chat_id_) or " لايوجد "
...
return sendMsg(msg.chat_id_,msg.id_,"- المالك :\n["..v.user.first_name.."](t.me/"..(v.user.username or "GLOBLA"))
end
end

message = ""
local monsha = redis:smembers(veer..':Malk_Group:'..msg.chat_id_)
if #monsha == 0 then 
...
elseif redis:sismember(veer..':Malk_Group:'..arg.ChatID,UserID) then 
...
if not redis:get(veer.."lock_linkk"..msg.chat_id_) then return "- الامر معطل من قبل الادارة ^"  end
if not redis:get(veer..'linkGroup'..msg.chat_id_) then return "- لا يوجد رابط \n- لانشاء رابط ارسل { انشاء رابط } ؛" end
...
local Admins = redis:scard(veer..':Malk_Group:'..msg.chat_id_)
if Admins == 0 then  
return "هناك خطا \n- عذرا لا يوجد المالكين ليتم مسحهم ✓" 
end
redis:del(veer..':Malk_Group:'..msg.chat_id_)
...
ReUsername = ResolveUserName(data)
NameUser = Hyper_Link_Name(data)
if redis:sismember(veer..':Malk_Group:'..arg.ChatID,arg.UserID) then 
sendMsg(arg.ChatID,arg.MsgID,"- المستخدم  ⋙「 "..NameUser.." 」 \n- تم بالتاكيد رفعه مالك  في المجموعه") 
else
sendMsg(arg.ChatID,arg.MsgID,"- المستخدم  ⋙「 "..NameUser.." 」 \n- تم رفعه مالك  في المجموعه") 
redis:hset(veer..'username:'..arg.UserID,'username',ReUsername)
redis:sadd(veer..':Malk_Group:'..arg.ChatID,arg.UserID)
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
end
...
local UserID = data.id_
NameUser = Hyper_Link_Name(data)
if redis:sismember(veer..':Malk_Group:'..arg.ChatID,UserID) then 
sendMsg(arg.ChatID,arg.MsgID,"- المستخدم  ⋙「 "..NameUser.." 」 \n- تم بالتاكيد رفعه مالك  في المجموعه") 
else
redis:hset(veer..'username:'..UserID,'username',arg.UserName)
redis:sadd(veer..':Malk_Group:'..arg.ChatID,UserID)
sendMsg(arg.ChatID,arg.MsgID,"- المستخدم  ⋙「 "..NameUser.." 」 \n- تم رفعه مالك  في المجموعه") 
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
end 
...
if not redis:sismember(veer..':Malk_Group:'..arg.ChatID,arg.UserID) then 
sendMsg(arg.ChatID,arg.MsgID,"- المستخدم  ⋙「 "..NameUser.." 」 \n- تم بالتاكيد تنزيله مالك  في المجموعه") 
else
sendMsg(arg.ChatID,arg.MsgID,"- المستخدم  ⋙「 "..NameUser.." 」 \n- تم تنزيله مالك  في المجموعه") 
redis:srem(veer..':Malk_Group:'..arg.ChatID,arg.UserID)
end  
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
end
...
UserName = Flter_Markdown(arg.UserName)
NameUser = Hyper_Link_Name(data)
if not redis:sismember(veer..':Malk_Group:'..arg.ChatID,UserID) then 
sendMsg(arg.ChatID,arg.MsgID,"- المستخدم  ⋙「 "..NameUser.." 」 \n- تم بالتاكيد تنزيله مالك  في المجموعه") 
else
redis:srem(veer..':Malk_Group:'..arg.ChatID,UserID)
sendMsg(arg.ChatID,arg.MsgID,"- المستخدم  ⋙「 "..NameUser.." 」 \n- تم تنزيله مالك  في المجموعه")
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
end 
...
elseif redis:sismember(veer..':SUDO_BOT:',UserID) then 
rinkuser = 2
elseif redis:sismember(veer..':Malk_Group:'..msg.chat_id_,UserID) then 
rinkuser = 3
elseif redis:sismember(veer..':MONSHA_Group:'..msg.chat_id_,UserID) then 
rinkuser = 4
elseif redis:sismember(veer..':MONSHA_BOT:'..msg.chat_id_,UserID) then 
...
if redis:sismember(veer..':Malk_Group:'..msg.chat_id_,UserID) then 
...
redis:srem(veer..':Malk_Group:'..msg.chat_id_,UserID)
...
redis:setex(veer..'welcom_ph:witting'..msg.sender_user_id_..msg.chat_id_,300,true) 
...
sendPhoto(msg.chat_id_,msg.id_,Photo_Weloame,[[- مـرحبا انا بوت اسـمـي ]]..redis:get(veer..':NameBot:')..[[ 🎖
...
return'- شـرط التفعيل البوت اذا كانت المجموعه‏‏ اكثر مـن *【'..redis:get(veer..':addnumberusers')..'】* عضـو  \n' 
end 
end
...
if not msg.SudoBase then return "- هذا الامر يخص {المطور الاساسي} فقط  \n" end
local GetVerison = https.request('https://GLOBLA.github.io/GetVersion.txt') or "0"
...
local inline = {{{text="قناه‏‏ السـورس : الـزعـيـم ",url="t.me/GLOBLA"}}}
send_key(msg.sender_user_id_,'  [قناة السورس: الـزعـيـم](t.me/GLOBLA)',nil,inline,msg.id_)
return false
end
...
if msg.SudoBase and (MsgText[1]=="م1" or MsgText[1]=="م2" or MsgText[1]=="م3" or MsgText[1]=="م المطور" or MsgText[1]=="اوامر الرد" or MsgText[1]=="الاوامر" or MsgText[1]=="اوامر الملفات") and redis:get(veer..":Witting_awamr_witting"..msg.chat_id_..msg.sender_user_id_) then 
...
- VEER TEAM 
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
- [Source Channel](https://t.me/GLOBLA)

- [Source Install](https://t.me/GLOBLA/4121)

- [Changing ID Channel](https://t.me/GLOBLA_id/1)

- [Explanation Channel](https://t.me/GLOBLA /1)

┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
- [Source Programmer](https://t.me/YYBYY)

- [Twasl Bot Programmer](https://t.me/YYBYYBOT)
...
if not msg.SudoBase then return "- هذا الامر يخص {المطور الاساسي} فقط  \n" end
local Get_Files, res = https.request("https://GLOBLA.github.io/GetFiles.json")
print(Get_Files)
print(res)
if res == 200 then
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/GLOBLA/GLOBLA.github.io/master/plugins/"..FileName)
if res == 200 then
...
text = [[- مـرحبا انا بوت اسـمـي []]..redis:get(veer..':NameBot:')..[[] 🎖
- اختصـاصـي حمـايه‏‏ المـجمـوعات
- مـن السـبام والتوجيه‏‏ والتكرار والخ...
...
text = text:gsub("{البوت}",redis:get(veer..':NameBot:'))
...
NameUser = Hyper_Link_Name(data)
if UserID == 1056550431 or UserID == 266057334 then 
...
if not redis:sismember(veer..'bannedpv',UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"- المستخدم  ⋙「 ["..arg.UserName.."] 」 \n-  تم بالتاكيد الغاء الحظر  من التواصل") 
end
redis:srem(veer..'bannedpv',UserID)
...
if FwdUser == 1056550431 or FwdUser == 266057334 then 
return sendMsg(arg.ChatID,arg.id_,"- لا يمكنك حظر مطور السورس ؛") 
end
redis:hset(veer..'username:'..arg.FwdUser,'username',USERNAME)
if redis:sismember(veer..'bannedpv',FwdUser) then 
return sendMsg(arg.ChatID,arg.id_,"- المستخدم  ⋙「 ["..USERNAME.."] 」 \n-  تم بالتاكيد حظره  من التواصل") 
end
redis:sadd(veer..'bannedpv',arg.FwdUser)
...
GetUserID(FwdUser,function(arg,data)
if data.username_ then USERNAME = '@'..data.username_ else USERNAME = FlterName(data,20) end
if not redis:sismember(veer..'bannedpv',arg.sender_user_id_) then 
return sendMsg(arg.ChatID,arg.id_,"- المستخدم  ⋙「 ["..USERNAME.."] 」 \n-  تم بالتاكيد الغاء حظره  من التواصل") 
end
redis:srem(veer..'bannedpv',arg.sender_user_id_)
...
if redis:sismember(veer..'bannedpv',msg.sender_user_id_) then  return false end
...
if msg.content_.caption_ then redis:hset(veer..':caption_replay:',klma,msg.content_.caption_) end
if msg.text then
...
redis:set(veer..":RtbaNew8:"..msg.chat_id_,msg.text)
...
if msg.content_.ID == "MessagePhoto" and redis:get(veer..'welcom_ph:witting'..msg.sender_user_id_..msg.chat_id_) then
redis:del(veer..'welcom_ph:witting'..msg.sender_user_id_..msg.chat_id_)
if msg.content_.photo_.sizes_[3] then 
photo_id = msg.content_.photo_.sizes_[3].photo_.persistent_id_
else 
photo_id = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
welcome = welcome:gsub("{القوانين}", redis:get(veer..'rulse:msg'..msg.chat_id_) or "- مرحبا عزيري  القوانين كلاتي \n- ممنوع نشر الروابط \n- ممنوع التكلم او نشر صور اباحيه \n- ممنوع  اعاده توجيه \n- ممنوع التكلم بلطائفه \n- الرجاء احترام المدراء والادمنيه \n")
...
local welcome = welcome:gsub("{البوت}",redis:get(veer..':NameBot:'))
...
GetUserID(msg.sender_user_id_,function(arg,data)
welcome = (redis:get(veer..'welcome:msg'..msg.chat_id_) or "- مرحباً عزيزي\n- نورت المجموعة ")
welcome = welcome:gsub("{القوانين}", redis:get(veer..'rulse:msg'..msg.chat_id_) or "- مرحبا عزيري  القوانين  \n- ممنوع نشر الروابط \n- ممنوع التكلم او نشر صور اباحيه \n- ممنوع  اعاده توجيه \n- ممنوع التكلم بلطائفه \n- الرجاء احترام المدراء والادمنيه \n")
...
return false
elseif (msg.text:match("ی") or msg.text:match('چ') or msg.text:match('گ') or msg.text:match('ک') or msg.text:match('پ') or msg.text:match('ژ') or msg.text:match('ٔ') or msg.text:match('۴') or msg.text:match('۵') or msg.text:match('۶')) and redis:get(veer.."lock_pharsi"..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
return false
elseif redis:get(veer.."lock_mmno3"..msg.chat_id_) and KlmatMmno3(msg.text)  then
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
if Replay then 
Caption = redis:hget(veer..':caption_replay:',msg.text)
Caption = convert_Klmat(msg,data,Caption)
print(Caption)
