...
download_file('https://raw.githubusercontent.com/DevGlobla/VEER/master/inc/Run.lua','./inc/Run.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'40% - |█████       |') end
download_file('https://raw.githubusercontent.com/DevGlobla/VEER/master/inc/locks.lua','./inc/locks.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'60% - |███████     |') end
download_file('https://raw.githubusercontent.com/DevGlobla/VEER/master/inc/Script.lua','./inc/Script.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'80% - |█████████   |') end
download_file('https://raw.githubusercontent.com/DevGlobla/VEER/master/inc/functions.lua','./inc/functions.lua')
...
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(veer..":NameBot:") or "سبارتا"
...
usernn = data.username_ or "spirta"
else 
Name = data.title_
usernn = data.type_.user_.username_ or "spirta"
end
Name = Name:gsub('[[][]]','')
...
var = redis:get(veer..":RtbaNew1:"..ChatID) or 'مطور اساسي ' 
...
var = redis:get(veer..":RtbaNew5:"..ChatID) or 'مدير البوت ' 
...
var = redis:get(veer..":RtbaNew7:"..ChatID) or 'عضو مميز ' 
else
var = 'فقط عضو ' 
end
return var
...
if #monshaas==0 and #monsha==0 and #Owners==0 and #Admins==0 and #mmez==0 then return " لا يوجد قائمه حاليا" end
i = 1
...
message = message ..i.. '-l ['..info.username..'](t.me/DevGlobla) \n'
end
...
message = message ..k.. '-l ['..info.username..'](t.me/DevGlobla) l » (`' ..v.. '`){'..count..'} \n'
end
end 
end
...
message = message ..k.. '-l ['..info.username..'](t.me/DevGlobla) l » (`' ..v.. '`) \n'
end
end
end
...
return sendMsg(arg.chat_id_,arg.id_,"⌔︙ تم رفع  { *"..NumAdmin.."* } مـن الادمـنيهہ‌‏ في البوت ")
end
...
if not msg.SudoUser and not lock_servicez then return '⌔︙لتفعيل حماية بوت بباي \n⌔︙عليك مراسلة مطور البوت @je17rebot' end
...
if redis:get(veer..'group:add'..msg.chat_id_) then  return sendMsg(msg.chat_id_,msg.id_,'⌔︙ المجموعه بالتأكيد تم تفعيلها \n⌔︙ بواسطه ←'..NameUser..' \n') end
...
return sendMsg(arg.chat_id_,arg.id_,'⌔︙ لا يمـكنني تفعيل البوت في المـجمـوعه يجب ان يكون اكثر مـن *【'..GroupUsers..'】* عضـو ')
end
...
sendMsg(arg.chat_id_,arg.id_,'⌔︙ تـم تـفـعـيـل الـمـجـمـوعـه  \n⌔︙ وتم رفع جمـيع الادمـنيه الكروب بالبوت \n⌔︙ بواسطه ←'..NameUser..' \n')
else
sendMsg(arg.chat_id_,arg.id_,'⌔︙ تـم تـفـعـيـل المـجمـوعه‏ \n⌔︙ بواسطه ←'..NameUser..' \n')
end
...
GetUserID(v.user_id_,function(arg,data)
redis:hset(veer..'username:'..data.id_,'username', ResolveUserName(data))
redis:sadd(veer..':MONSHA_Group:'..arg.chat_id_,data.id_)
...
function senddwon()  sendMsg(ChatID,MsgID,"⌔︙ عذرا العضو رتبته اعلى منك لا يمكن تنزيله ") end
...
local eueuf = "\n\n*⚒¦*` اعدادات اخرى : `"
.."\n*🙋🏼‍♂️¦* الترحيب » "..(redis:get(veer..'welcome:get'..msg.chat_id_) or 'false')
